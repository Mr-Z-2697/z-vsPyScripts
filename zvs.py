__version__=str(1789309873/2**31)
import os,sys
...
'''
def zmud(src,tr=None,thsad=100,thsadc=None,blksize=16,mv_pad=None,resize_pad=True,overlap=None,pel=1,chromamv=True,sharp=2,rfilter=2,satd=0,thscd1=400,thscd2=51,pref=None,cs=False,csrad=1,csrep=14,cspl=None,refinemotion=False,rmblksize=None,rmoverlap=None,rmpel=None,rmchromamv=None,rmthsad=None,rmsatd=None,mvout=False,mvout_sup=False,mvin=None,mvinrm=False,mvupd=None,limit=None,lf=None,elast=20,sargs={},aargs={},rargs={},alim_ref=None,alim_cdif=False,**args):
    if resize_pad:
    ...
    if mvd_in:
        ...
        if mvinrm:
            mvs=core.mvu.Recalculate(sup3,_mvs,thsad=rmthsad,blksize=rmblksize,overlap=rmoverlap,chroma=rmchromamv,satd=rmsatd,**rargs)
            if mvupd:
                mvin['mvs']=mvs
        else:
            mvs=_mvs
    else:
        mvs=core.mvu.AnalyseMany(sup,radius=tr,blksize=blksize,overlap=overlap,chroma=chromamv,satd=satd,**aargs)
        if refinemotion:
            mvs=core.mvu.Recalculate(sup3,mvs,thsad=rmthsad,blksize=rmblksize,overlap=rmoverlap,chroma=rmchromamv,satd=rmsatd,**rargs)
    ...
    last=core.mvu.Degrain(last,_sup,mvs,thsad=[thsad,thsadc],thscd1=thscd1,thscd2=thscd2,limit=limit)
    if alim:
...
def hrife(src,ref=None,mode=None,m='709',format=None,rgbh=True,base=64,pad=1,color=None,resizer=None):
    ...
    if pad:
        if mode=='i' or src.format.color_family==vs.YUV:
            ...
            ret=resizer(ret,format=[vs.RGBS,vs.RGBH][rgbh],matrix_in_s=m)
        elif mode=='o' or src.format.color_family==vs.RGB:
            ret=resizer(src,format=format,matrix_s=m)
            ret=ret.std.Crop(0,p_w-s_w,0,p_h-s_h)
    else:
        if mode=='i' or src.format.color_family==vs.YUV:
            ret=setmatrix(src,'rm')
            ret=resizer(ret,p_w,p_h,src_width=p_w,src_height=p_h,format=[vs.RGBS,vs.RGBH][rgbh],matrix_in_s=m)
        elif mode=='o' or src.format.color_family==vs.RGB:
            ret=resizer(src,s_w,s_h,src_width=s_w,src_height=s_h,format=format,matrix_s=m)
    return ret
...
def gaussianblurfmtc(src,sigma=1,stdev=None,mode='impulse',planes=[0,1,2],r=None,impext=3,impdr=1,kd='bilinear',rsa1=9,sci=False,dir='hv',is444=None):
    ...
    if mode=='resample': #by dnjulek at https://github.com/sekrit-twc/zimg/issues/186#issue-1462626153
        ...
        if sci:
            ...
            _gd=lambda x:math.exp(-.5*(x/stdev)**2)/(math.sqrt(math.pi*2)*stdev)
...
def bm3d_core(clip,ref=None,mode="cpu",sigma=3.0,block_step=8,bm_range=9,radius=0,ps_num=2,ps_range=4,chroma=False,fast=True,extractor_exp=bm3d_extractor_exp_default,device_id=0,bm_error_s="SSD",transform_2d_s="DCT",transform_1d_s="DCT",vt=0):
    ...
    if mode not in ["cpu","cuda","cuda_rtc"]:
        ...
        if vt==1:
            fullres=core.bm3dcpu.BM3Dv2(clip,ref=ref,sigma=sigma,block_step=block_step,bm_range=bm_range,radius=radius,ps_num=ps_num,ps_range=ps_range,chroma=chroma_fullres)
        else:
            fullres=core.bm3dcpu.BM3D(clip,ref=ref,sigma=sigma,block_step=block_step,bm_range=bm_range,radius=radius,ps_num=ps_num,ps_range=ps_range,chroma=chroma_fullres)
    elif mode=="cuda":
        if vt==1:
            fullres=core.bm3dcuda.BM3Dv2(clip,ref=ref,sigma=sigma,block_step=block_step,bm_range=bm_range,radius=radius,ps_num=ps_num,ps_range=ps_range,chroma=chroma_fullres,fast=fast,extractor_exp=extractor_exp,device_id=device_id)
        else:
            fullres=core.bm3dcuda.BM3D(clip,ref=ref,sigma=sigma,block_step=block_step,bm_range=bm_range,radius=radius,ps_num=ps_num,ps_range=ps_range,chroma=chroma_fullres,fast=fast,extractor_exp=extractor_exp,device_id=device_id)
    else:
        if vt==1:
